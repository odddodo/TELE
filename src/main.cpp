...
struct Chan
{
    float nsclx, nscly, tscl, sinscl, mask, steepness;
};
...
IRAM_ATTR void generateNoiseFrame()
{
    ...
    for (int x = 0; x < W; x++)
    {
        ...
        for (int y = 0; y < H; y++)
        {
            ...
            uint8_t palIdx = (uint8_t)(((uint16_t)(sin8(v_res) * cos8(v_res)) / (1 + (uint8_t)cx.mask) +
                                        (uint16_t)(sin8(k_res) * cos8(k_res)) / (1 + (uint8_t)cy.mask)) /
                                       2);
